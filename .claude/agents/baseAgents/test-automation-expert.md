...
description: 'Designs and implements robust, fast, and maintainable automated tests across unit, API, UI, performance, and security layers. Use when planning a test automation strategy, writing or reviewing tests at any layer, or diagnosing test suite quality issues.'
...
- [ ] **Implement & Test** – small commits, run tests via `Bash`, analyze test output, mitigate flakes, improve observability (logs/traces/metrics).
