...
description: 'Generates production-quality REST API tests from an OpenAPI specification in any language or framework. Use when creating automated API tests from an OpenAPI/Swagger file, covering happy paths, validation errors, and auth scenarios.'
tools:
  - Bash
  - Read
  - Glob
...
Given an OpenAPI spec (file path or URL) AND user-provided details generate production-quality automated API tests that:
...
- language: the programming language (if not provided, use what is used in project)
...
- if URL, fetch it using `WebFetch`; if file path, read it from disk using `Read`
